...
func HttpResult(r *http.Request, w http.ResponseWriter, resp interface{}, err error) {
	...
	if err == nil {
		//成功返回
		re := Success(resp)
		logx.WithContext(r.Context()).Errorf("【RPC-SRV-ERR】 %+v", "111")
		httpx.WriteJson(w, http.StatusOK, re)
	} else {
		//错误返回
		errcode := errorx.REQUEST_ERROR
		errmsg := errorx.MapErrMsg(errorx.REQUEST_ERROR)
...
func ParamErrorResult(r *http.Request, w http.ResponseWriter, err error) {
	logx.WithContext(r.Context()).Errorf("【ParamErrorResult】")
	errMsg := fmt.Sprintf("%s ,%s", errorx.MapErrMsg(errorx.REQUEST_PARAM_ERROR), err.Error())
	httpx.WriteJson(w, http.StatusBadRequest, Error(errorx.REQUEST_PARAM_ERROR, errMsg))
}
